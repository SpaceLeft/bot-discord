...
module.exports = {
  ...
  description: 'See a list of words on the words-list.',
    run: async (client, message) => {
        ...
        let wordlist = new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setColor("9e1c36").setTitle(`${message.guild.name} AnitWords List`).setThumbnail(guild)
